...
def icon_lollipop(ax):
    ax.lollipop(CATEGORIES, VALUES, marker="o", markersize=9, lw=2, color=ICON_LINE)


def icon_lollipoph(ax):
    ax.lollipoph(CATEGORIES, VALUES, marker="o", markersize=9, lw=2, color=ICON_LINE)
...
def icon_graph(ax):
    ...
    ax.graph(
        ...
        layout="spring",
        layout_kw={"seed": 4},
        node_kw={"node_size": 35, "node_color": ICON_LINE, "linewidths": 0},
        edge_kw={"alpha": 0.55, "width": 1.1},
...
def icon_radar(ax):
    ...
    with without_new_text(ax):
        ax.radar_chart(frame, vmin=0, vmax=5, fill=True, marker_size=5)
...
def slug(name):
    ...
    which belong in a path that draw.io and Sphinx both have to reference.
    """
    name = name.strip()
    for old, new in (
        ("(", "-"),
...
def main():
    ...
    for name, (draw, proj, _kind, _mpl, _group) in ICONS.items():
        ...
        save(fig, f"icons/{slug(name)}.svg", dpi=220)
    if failures:
        print("icon failures:")
