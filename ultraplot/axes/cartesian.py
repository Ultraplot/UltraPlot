...
class CartesianAxes(shared._SharedAxes, plot.PlotAxes):
    ...
    def _apply_axis_sharing(self):
        ...
        if self._sharex is not None and self.xaxis.get_visible():
            # If we are sharing with a panel
            # turn all labels off
            ...
            if level > 2:
                ...
                ticks_visible = dict(labeltop=False, labelbottom=False)
                self.xaxis.set_tick_params(which="both", **ticks_visible)

        if self._sharey is not None and self.yaxis.get_visible():
            ...
            if level > 2:
                ticks_visible = dict(labelleft=False, labelright=False)
                self.yaxis.set_tick_params(which="both", **ticks_visible)
