...
def test_add_subplots_fortran_order():
    """Fortran order ('F') arranges subplots column-major."""
    fig = uplt.figure()
...
def test_add_subplots_gridspec_kw_warns():
    ...
    with pytest.warns(match="gridspec_kw"):
        axs = fig.add_subplots(nrows=1, ncols=2, gridspec_kw={"wspace": 1})
    uplt.close(fig)
...
def test_add_subplot_external_projection_wrapped():
    """Non-ultraplot projections (e.g. 'hammer') get wrapped in a container."""
    fig = uplt.figure()
    ax = fig.add_subplot(111, proj="hammer")
    assert ax is not None
    assert hasattr(ax, "number")
    uplt.close(fig)
